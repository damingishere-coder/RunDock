...
export default function AnalyticsPage({ processes, settings, reload }: Props) {
  ...
  async function startNamespace(ns: string) {
    ...
    setBusyAction(`namespace:${ns}`)
    try {
      const result = await api.startNamespace(ns)
      setActionError(namespaceStartError(ns, result))
  ...
  async function restartCrashed() {
    if (actionInFlightRef.current) return
    actionInFlightRef.current = true
    setBusyAction('restart-crashed')
    try {
      const results = await mapSettledWithConcurrency(
        processes.filter(p => p.status === 'crashed' || p.status === 'errored'),
        4,
        process => api.restartProcess(process.id)
      )
      const failed = results.filter(result => result.status === 'rejected').length
      setActionError(failed ? `${failed} 个崩溃进程重启失败` : null)
      setTimeout(reload, 500)
    } catch (error) {
      setActionError(error instanceof Error ? error.message : '批量重启崩溃进程失败')
    } finally {
      actionInFlightRef.current = false
      setBusyAction(null)
    }
  ...
  const stats = useMemo(() => {
    ...
    for (const p of processes) {
      ...
      r.total++
      r.totalCpu += p.cpu_percent ?? 0
      r.totalMem += p.memory_bytes ?? 0
    ...
    const statusSegments: StatusSegment[] = (
      [
        ...
        },
        {
          ...
          count: stopped + stopping,
          color: STATUS_COLORS.stopped,
          label: '已停止',
        },
        {
          status: 'crashed' as const,
          count: crashed + errored,
          color: STATUS_COLORS.crashed,
          label: '已崩溃',
        },
        {
  ...
  const hasLeaderData =
    stats.longestRunning.length > 0 || stats.topMemory.length > 0 || stats.topRestarters.length > 0
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ flexShrink: 0, borderBottom: '1px solid var(--color-border)' }}>
        ...
        {(actionError || healthError || healthWarning) && (
          <div
            role="alert"
            style={{
              padding: '7px 24px',
              color: 'var(--color-status-crashed)',
              background: 'color-mix(in srgb, var(--color-status-crashed) 9%, transparent)',
              borderTop:
                '1px solid color-mix(in srgb, var(--color-status-crashed) 35%, transparent)',
              fontSize: 12,
            }}
          >
            {actionError ??
              (healthError ? `${healthError}；系统卡片保留上次成功数据。` : healthWarning)}
          </div>
        )}
        ...
        <div style={{ display: 'flex', borderTop: '1px solid var(--color-border)' }}>
          ...
          <StatCell
            n={stats.activeCount}
            label="运行中"
            c={stats.activeCount > 0 ? 'var(--color-status-running)' : undefined}
          />
          <StatCell n={stats.stopped + stats.stopping} label="已停止" />
          <StatCell
            n={stats.crashed + stats.errored}
            label="已崩溃"
            c={hasCrashed ? '#ef4444' : undefined}
          />
          <div style={{ flex: 1 }} />
          <StatCell n={formatCpu(stats.totalCpu)} label="CPU" c="#60a5fa" />
          <StatCell n={formatBytes(stats.totalMem)} label="内存" c="#a78bfa" />
          <StatCell
            n={String(stats.totalRestarts)}
            label="重启次数"
            c={stats.totalRestarts > 0 ? '#f59e0b' : undefined}
          />
        </div>
      </div>
              ...
              >
                ...
                <span style={{ flex: 1, fontSize: 12.5, color: '#ef4444' }}>
                  ...
                  <span style={{ fontWeight: 400, marginLeft: 6, opacity: 0.8 }}>
                    — {stats.crashedList.map(p => p.name).join(', ')}
                  </span>
                </span>
                <button
                  onClick={restartCrashed}
                  disabled={busyAction !== null}
                  style={{ ...dangerBtn, opacity: busyAction !== null ? 0.55 : 1 }}
                >
                  <RotateCcw size={10} />
                  {busyAction === 'restart-crashed' ? '重启中…' : '全部重启'}
                </button>
                <button onClick={() => navigate('/processes')} style={ghostBtn}>
                  查看
                </button>
              </div>
            )}
                ...
                >
                  ...
                  {stats.namespaces.map((ns, i) => (
                    <NsListRow
                      key={ns.namespace}
                      ns={ns}
                      last={i === stats.namespaces.length - 1}
                      ...
                      starting={busyAction === `namespace:${ns.namespace}`}
                    />
                  ))}
                </div>
              )}
                ...
                {stats.topMemory.length > 0 && (
                  <LeaderPanel icon={MemoryStick} title="内存占用最高">
                    {stats.topMemory.map((p, i) => {
                      const pct =
                        stats.totalMem > 0 ? ((p.memory_bytes ?? 0) / stats.totalMem) * 100 : 0
                      return (
                        <div
                          key={p.id}
                          onClick={() => navigate(`/processes/${p.id}`)}
                  ...
                  {stats.statusSegments.length === 0 ? (
                    ...
                    stats.statusSegments.map(s => (
                      <div key={s.status} style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                        ...
                        />
                        <span
                          style={{ flex: 1, fontSize: 12, color: 'var(--color-muted-foreground)' }}
                        >
                          {s.label}
                        </span>
            ...
            {health && (
              <div style={sidePanel}>
                ...
                <div style={{ marginTop: 10, display: 'flex', flexDirection: 'column' }}>
                  <KV
                    k="状态"
                    v={health.status === 'ok' ? '正常' : '持久化异常'}
                    vc={health.status === 'ok' ? 'var(--color-status-running)' : '#ef4444'}
                  />
                  ...
                  <KV
                    k="活动进程"
                    v={String(stats.activeCount)}
                    vc={stats.activeCount > 0 ? 'var(--color-status-running)' : undefined}
                  />
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
        ...
        >
          <div
            style={{ width: `${pct}%`, height: '100%', background: dot, transition: 'width 0.4s' }}
          />
        </div>
      ...
      <span
        style={{
          fontSize: 12,
          color: ns.totalCpu > 0 ? '#60a5fa' : 'var(--color-muted-foreground)',
          textAlign: 'right',
          fontVariantNumeric: 'tabular-nums',
        }}
      >
      ...
      <span
        style={{
          fontSize: 12,
          color: ns.totalMem > 0 ? '#a78bfa' : 'var(--color-muted-foreground)',
          textAlign: 'right',
          fontVariantNumeric: 'tabular-nums',
        }}
      >
      ...
      >
        {(ns.stopped > 0 || ns.crashed > 0) && (
          <button onClick={onStart} disabled={actionBusy} style={rowBtn}>
            <Play size={9} />
            {starting ? '启动中…' : '启动'}
          </button>
        )}
  ...
  return (
    <svg width={s} height={s}>
      ...
      {segments.map((seg, i) => {
        ...
        const precedingCount = segments
          .slice(0, i)
          .reduce((sum, preceding) => sum + preceding.count, 0)
        const angle = (precedingCount / total) * 360 - 90
        return (
          <circle
            key={i}
            cx={cx}
...
function LRow({
  ...
  return (
    ...
    >
      ...
      <span style={{ fontSize: 12, fontWeight: 700, color: mc, minWidth: 44, textAlign: 'right' }}>
        {metric}
      </span>
    </button>
  )
...
function KV({ k, v, vc, last }: { k: string; v: string; vc?: string; last?: boolean }) {
  return (
    ...
    >
      ...
      <span style={{ fontSize: 12, fontWeight: 600, color: vc ?? 'var(--color-foreground)' }}>
        {v}
      </span>
    </div>
  )
...
function Muted({ children }: { children: ReactNode }) {
  return (
    <p style={{ fontSize: 12, color: 'var(--color-muted-foreground)', margin: '8px 0 0' }}>
      {children}
    </p>
  )
}
...
function LogStatsSection({ processes }: { processes: ProcessInfo[] }) {
  ...
  const fetchAll = useCallback(
    async (isCurrent: () => boolean, signal: AbortSignal) => {
      ...
      const results = await mapSettledWithConcurrency(
        processes,
        6,
        async p => [p.id, (await api.getLogStats(p.id, { signal })).buckets] as const
      )
      if (!isCurrent()) return
      ...
      setStatsMap(previous => ({
        ...Object.fromEntries(Object.entries(previous).filter(([id]) => currentIds.has(id))),
        ...Object.fromEntries(entries),
      }))
      const failed = results.length - entries.length
      setStatsError(failed ? `${failed} 个进程的日志统计暂时不可用` : null)
      if (failed) throw new Error(`${failed} log-stat requests failed`)
    },
    [processes]
  )
  useSingleFlightPoll(fetchAll, { intervalMs: 60_000, refreshKey: ids })

  const withData = processes.filter(p => (statsMap[p.id]?.length ?? 0) > 0)
  if (!withData.length) return statsError ? <Muted>{statsError}</Muted> : null
  ...
  return (
    ...
    >
      ...
      >
        {withData.map(p => (
          <LogCard key={p.id} process={p} buckets={statsMap[p.id] ?? []} />
        ))}
      </div>
      {statsError && <Muted>{statsError}</Muted>}
    </div>
  )
}
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginBottom: 6 }}>
        ...
        <span style={{ fontSize: 11, color: 'var(--color-status-running)', fontWeight: 600 }}>
          {totalOut} out
        </span>
        <span style={{ fontSize: 10, color: 'var(--color-muted-foreground)', margin: '0 2px' }}>
          ·
        </span>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: H, display: 'block' }}>
        ...
        {buckets.map((b, i) => {
          const x = i * (bW + gap)
          const t = b.stdout_count + b.stderr_count
          const tH = (t / maxCount) * H
          const oH = (b.stdout_count / maxCount) * H
          const eH = tH - oH
          return (
            <g key={i}>
              {eH > 0 && (
...
const nsHeaderRow: CSSProperties = {
  display: 'grid',
  gridTemplateColumns: '28px 1fr 160px 70px 80px 90px',
  alignItems: 'center',
  gap: 12,
  padding: '6px 16px',
