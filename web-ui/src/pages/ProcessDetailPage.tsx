...
function parseStreamLogLine(raw: string): LogLine | null {
  let value: unknown
  try {
  ...
  if (
    ...
    (line.timestamp !== undefined && typeof line.timestamp !== 'string')
  ) {
    return null
  }
  return {
...
function isLogLineArray(value: unknown): value is LogLine[] {
  return (
    Array.isArray(value) &&
    value.every(
      line =>
        ...
        line !== null &&
        !Array.isArray(line) &&
        ((line as Record<string, unknown>).stream === 'stdout' ||
          (line as Record<string, unknown>).stream === 'stderr') &&
        typeof (line as Record<string, unknown>).timestamp === 'string' &&
        typeof (line as Record<string, unknown>).content === 'string'
    )
  )
}

function isLogStatsArray(value: unknown): value is LogStatsBucket[] {
  return (
    Array.isArray(value) &&
    value.every(
      bucket =>
        ...
        typeof (bucket as Record<string, unknown>).window_start === 'string' &&
        typeof (bucket as Record<string, unknown>).stdout_count === 'number' &&
        Number.isFinite((bucket as Record<string, unknown>).stdout_count as number) &&
        typeof (bucket as Record<string, unknown>).stderr_count === 'number' &&
        Number.isFinite((bucket as Record<string, unknown>).stderr_count as number)
    )
  )
}

function isMetricSampleArray(value: unknown): value is MetricSample[] {
  return (
    Array.isArray(value) &&
    value.every(
      sample =>
        ...
        typeof (sample as Record<string, unknown>).timestamp === 'string' &&
        typeof (sample as Record<string, unknown>).cpu_percent === 'number' &&
        Number.isFinite((sample as Record<string, unknown>).cpu_percent as number) &&
        typeof (sample as Record<string, unknown>).memory_bytes === 'number' &&
        Number.isFinite((sample as Record<string, unknown>).memory_bytes as number)
    )
  )
}
...
export default function ProcessDetailPage({ reload, settings, onOpenTerminal }: Props) {
  ...
  const [envTarget, setEnvTarget] = useState<{ id: string; name: string } | null>(null)
  ...
  const currentProcessIdRef = useRef(id)
  currentProcessIdRef.current = id
  ...
  // Load process info + poll sequentially at the configured interval.
  const loadProcess = useCallback(
    async (isCurrent: () => boolean, signal: AbortSignal) => {
      if (!id) return
      try {
        const data = await api.getProcess(id, { signal })
        if (isCurrent()) {
          if (!isProcessInfo(data) || data.id !== id) {
            throw new Error('服务端返回了无效或其他进程的详情')
          }
          setProcess(data)
          setLoadError(null)
        }
      } catch (error) {
        if (isCurrent()) setLoadError(error instanceof Error ? error.message : '加载进程失败')
        throw error
      }
    },
    [id]
  )
  useSingleFlightPoll(loadProcess, {
    intervalMs: settings.processRefreshInterval,
    refreshKey: id ?? '',
  })
  ...
  useEffect(() => {
    ...
    api
      .getLogDates(id, { signal: abort.signal })
      .then(d => {
        if (!Array.isArray(d.dates) || !d.dates.every(date => typeof date === 'string')) {
          throw new Error('服务端返回了无效的日志日期')
        }
        if (current) setLogDates(d.dates)
      })
      .catch(error => {
        if (current) setLoadError(error instanceof Error ? error.message : '加载日志日期失败')
      })
    return () => {
      current = false
      abort.abort()
  ...
  const loadStats = useCallback(
    async (isCurrent: () => boolean, signal: AbortSignal) => {
      if (!id) return
      try {
        const data = await api.getLogStats(id, { signal })
        if (!isLogStatsArray(data.buckets)) throw new Error('服务端返回了无效的日志统计')
        if (isCurrent()) setLogStats(data.buckets)
      } catch (error) {
        if (isCurrent()) setLoadError(error instanceof Error ? error.message : '加载日志统计失败')
        throw error
      }
    },
    [id]
  )
  useSingleFlightPoll(loadStats, {
  ...
  const loadMetrics = useCallback(
    async (isCurrent: () => boolean, signal: AbortSignal) => {
      if (!id) return
      try {
        const data = await api.getMetricsHistory(id, { signal })
        if (!isMetricSampleArray(data.samples)) throw new Error('服务端返回了无效的历史指标')
        if (isCurrent()) setMetricSamples(data.samples)
      } catch (error) {
        if (isCurrent()) setLoadError(error instanceof Error ? error.message : '加载历史指标失败')
        throw error
      }
    },
    [id]
  )
  useSingleFlightPoll(loadMetrics, {
    intervalMs: 60 * 1000,
    refreshKey: id ?? '',
  })
  ...
  useEffect(() => {
    ...
    const abort = new AbortController()
    api
      .getProcessGit(id, { signal: abort.signal })
      .then(data => {
        if (current) setGitInfo(data)
      })
      .catch(error => {
        if (current) setLoadError(error instanceof Error ? error.message : '加载 Git 信息失败')
      })
    return () => {
      current = false
      abort.abort()
  ...
  useEffect(() => {
    if (!id) return
    let cancelled = false
    const streamTicketAbort = new AbortController()
    ...
    api
      .getLogs(
        id,
        {
          lines: isToday ? settings.logTailLines : Math.max(settings.logTailLines, 500),
          date: dateParam,
        },
        { signal: historyAbort.signal }
      )
      .then(d => {
        if (!isLogLineArray(d.lines)) throw new Error('服务端返回了无效的日志记录')
        if (!cancelled)
          setLogLines(
            d.lines.map(l => ({
              stream: l.stream,
      ...
      .catch(error => {
        if (!cancelled) setLoadError(error instanceof Error ? error.message : '加载日志失败')
      })

    // Live SSE for today only
    if (isToday) {
      api
        .streamLogs(id, { signal: streamTicketAbort.signal })
        .then(es => {
          if (cancelled) {
            es.close()
            return
          ...
          esRef.current = es
          es.onmessage = e => {
            const line = parseStreamLogLine(e.data)
            if (!line) {
              setLoadError('实时日志收到格式错误的数据，已跳过该条记录')
              return
            }
            setLogLines(prev => [...prev.slice(-4_999), line])
          }
          es.onerror = () => {
            es.close()
            esRef.current = null
            if (!cancelled) setLoadError('实时日志连接已中断，历史日志仍保留在页面中')
          }
        })
        .catch(error => {
          if (!cancelled) setLoadError(error instanceof Error ? error.message : '建立日志流失败')
        })
    }

    return () => {
      cancelled = true
      streamTicketAbort.abort()
      historyAbort.abort()
      if (esRef.current) {
        esRef.current.close()
        esRef.current = null
  ...
  useEffect(() => {
    if (atBottomRef.current) {
      ...
      queueMicrotask(() => setSliderPos(1000))
    }
  }, [logLines])
  ...
  if (!process || process.id !== id)
    return (
      <div
        style={{
          padding: 24,
          color: loadError ? 'var(--color-status-crashed)' : 'var(--color-muted-foreground)',
        }}
      >
        {loadError ?? '加载中…'}
      </div>
  ...
  const isActive =
    process.status === 'running' || process.status === 'sleeping' || process.status === 'watching'
  const isToday = dateIndex === -1
  ...
  async function doStart() {
    try {
      await api.startStopped(process!.id)
      setTimeout(reload, 300)
    } catch (actionError) {
      await alert('启动失败', actionError instanceof Error ? actionError.message : '进程未启动')
    }
  }
  async function doRestart() {
    try {
      await api.restartProcess(process!.id)
      reload()
    } catch (actionError) {
      await alert('重启失败', actionError instanceof Error ? actionError.message : '进程未重启')
    }
  }
  async function doStop() {
    ...
    if (!ok) return
    try {
      await api.stopProcess(process!.id)
      reload()
    } catch (actionError) {
      await alert('停止失败', actionError instanceof Error ? actionError.message : '进程未停止')
    }
  }
  async function doDelete() {
    ...
    try {
      ...
      await alert('删除失败', actionError instanceof Error ? actionError.message : '进程未删除')
    }
  }
  function doTerminal() {
  ...
  async function doGitPull() {
    const actionProcessId = process!.id
    setGitStatus('pulling')
    setGitLog(null)
    setGitLogOpen(false)
    try {
      const result = await api.gitPull(actionProcessId)
      if (currentProcessIdRef.current !== actionProcessId) return
      ...
      api
        .getProcessGit(actionProcessId)
        .then(data => {
          if (currentProcessIdRef.current === actionProcessId) setGitInfo(data)
        })
        .catch(error => {
          if (currentProcessIdRef.current === actionProcessId) {
            setLoadError(error instanceof Error ? error.message : '刷新 Git 信息失败')
          }
        })
  ...
  async function doOpenFolder() {
    ...
    try {
      await api.openFolder(process!.cwd)
    } catch (actionError) {
      await alert(
        '打开目录失败',
        actionError instanceof Error ? actionError.message : '无法打开工作目录'
      )
    }
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      ...
      >
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          ...
          <ToolBtn
            label="🔑 .env"
            onClick={() => setEnvTarget({ id: process.id, name: process.name })}
          />
            ...
            {process.cwd && (
              <button
                onClick={() => {
                  void (async () => {
                    try {
                      await navigator.clipboard.writeText(process.cwd!)
                      setCwdCopied(true)
                      setTimeout(() => setCwdCopied(false), 1500)
                    } catch (error) {
                      setLoadError(error instanceof Error ? error.message : '复制路径失败')
                    }
                  })()
                }}
      ...
      {gitInfo?.is_git_repo && (
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
            ...
            {/* Ahead / behind */}
            {gitInfo.behind > 0 && (
              <span style={{ fontSize: 10, color: 'var(--color-destructive)', fontWeight: 600 }}>
                ↓{gitInfo.behind} 个落后提交
              </span>
            )}
            {gitInfo.ahead > 0 && (
              <span style={{ fontSize: 10, color: 'var(--color-status-running)', fontWeight: 600 }}>
                ↑{gitInfo.ahead} 个领先提交
              </span>
            )}
            {!gitInfo.upstream_available && (
              <span
                title={gitInfo.ahead_behind_error ?? '未配置上游分支'}
            ...
            <button
              ...
              style={{
                ...
                borderRadius: 4,
                border: 'none',
                cursor: gitStatus === 'pulling' ? 'default' : 'pointer',
                background:
                  gitStatus === 'done'
          ...
          {gitLog && gitLogOpen && (
            <pre
              style={{
                ...
                wordBreak: 'break-all',
                color:
                  gitStatus === 'error' ? 'var(--color-destructive)' : 'var(--color-foreground)',
              }}
            >
              {gitLog}
            </pre>
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <span
            style={{ color: 'var(--color-muted-foreground)', minWidth: 100, textAlign: 'center' }}
          >
      ...
      {isToday && visibleLines.length > 1 && (
        ...
        >
          ...
          >
            ...
            <span
              style={{
                color: sliderPos >= 990 ? 'var(--color-status-running)' : 'var(--color-primary)',
              }}
            >
      ...
      >
        <span style={{ fontSize: 12, color: 'var(--color-muted-foreground)', userSelect: 'none' }}>
          🔍
        </span>
        <input
          type="text"
        ...
        {textFilter && (
          <>
            <span
              style={{ fontSize: 11, color: 'var(--color-muted-foreground)', whiteSpace: 'nowrap' }}
            >
...
function MetricsPanel({
  ...
  const todaySamples = samples.filter(
    s => new Date(s.timestamp).getTime() >= todayMidnight.getTime()
  )
            ...
            {(
              ...
              <button
                ...
                style={{
                  ...
                  color:
                    tab === t ? 'var(--color-primary-foreground)' : 'var(--color-muted-foreground)',
                }}
              >
                {label}
              </button>
...
function LogVolumeContent({ buckets }: { buckets: LogStatsBucket[] }) {
  ...
  const todayMidnight = new Date()
  todayMidnight.setHours(0, 0, 0, 0)
  const nowX = ((new Date().getTime() - todayMidnight.getTime()) / (24 * 60 * 60 * 1000)) * W
...
function CpuContent({ samples }: { samples: MetricSample[] }) {
  ...
  const nowX = ((new Date().getTime() - startMs) / dayMs) * W
  function toX(ts: string) {
    return ((new Date(ts).getTime() - startMs) / dayMs) * W
  }
  const pts = samples
    .map(
      s =>
        `${toX(s.timestamp).toFixed(1)},${(H - (Math.min(s.cpu_percent, 100) / 100) * H).toFixed(1)}`
    )
    .join(' ')
  const fill = samples.length
    ? `${toX(samples[0].timestamp).toFixed(1)},${H} ${pts} ${toX(samples[samples.length - 1].timestamp).toFixed(1)},${H}`
    : ''
...
function MemContent({ samples }: { samples: MetricSample[] }) {
  ...
  const pts = samples
    .map(s => `${toX(s.timestamp).toFixed(1)},${(H - (s.memory_bytes / maxMem) * H).toFixed(1)}`)
    .join(' ')
  const fill = samples.length
    ? `${toX(samples[0].timestamp).toFixed(1)},${H} ${pts} ${toX(samples[samples.length - 1].timestamp).toFixed(1)},${H}`
    : ''
