...
function ProcessCard({
  ...
  const errRate = totalOut + totalErr > 0 ? Math.round((totalErr / (totalOut + totalErr)) * 100) : 0
  ...
  return (
    <div
      ...
      role="link"
      tabIndex={0}
      aria-label={`查看进程 ${process.name} 的日志详情`}
      style={{
    ...
    >
      ...
      <div style={{ display: 'flex', gap: 12, fontSize: 11 }}>
        ...
        <span
          style={{
            color: totalErr > 0 ? 'var(--color-status-crashed)' : 'var(--color-muted-foreground)',
            fontWeight: totalErr > 0 ? 600 : 400,
          }}
        >
...
export default function LogVolumePage({ processes }: Props) {
  ...
  const fetchAll = useCallback(
    async (isCurrent: () => boolean, signal: AbortSignal) => {
      ...
      setLoadError(failed ? `${failed} 个进程的日志统计加载失败，已保留上次成功数据` : null)
  ...
  const reloadStats = useSingleFlightPoll(fetchAll, {
    intervalMs: 60_000,
    refreshKey: ids,
  })
  ...
  const aggregateBuckets = useMemo(() => {
    const relevant = Object.fromEntries(filtered.map(p => [p.id, statsMap[p.id] ?? []]))
  ...
  return (
    <div style={{ padding: '20px 24px', maxWidth: 1400, margin: '0 auto' }}>
      ...
      >
        <div>
          ...
          <p style={{ fontSize: 12, color: 'var(--color-muted-foreground)', margin: '4px 0 0' }}>
            ...
            {lastRefresh && <span> · 更新于 {fmtTime(lastRefresh.toISOString())}</span>}
          </p>
        </div>
            ...
            <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
              <span
                style={{
                  ...
                  height: 8,
                  borderRadius: 2,
                  background: 'var(--color-status-crashed)',
                  display: 'inline-block',
                }}
              />
              错误输出（stderr）
      ...
      {loadError && (
        <p
          role="alert"
          style={{ margin: '-10px 0 16px', color: 'var(--color-status-crashed)', fontSize: 12 }}
        >
          {loadError}
        </p>
      )}
      ...
      >
        {[
          {
            label: '标准输出总量',
            value: fmtCount(totals.stdout),
            sub: '今日日志行数',
            color: 'var(--color-status-running)',
          },
          {
            label: '错误输出总量',
            value: fmtCount(totals.stderr),
            sub: '今日日志行数',
          ...
          {
            label: '有日志的进程',
            value: String(withData),
            sub: `共 ${totalProcesses} 个进程`,
            color: 'var(--color-foreground)',
          },
        ].map(card => (
          <div
      ...
      <div
        style={{
          display: 'grid',
          gridTemplateColumns:
            aggregateBuckets.length > 0 && topProcesses.length > 0 ? '1fr 260px' : '1fr',
          gap: 16,
          marginBottom: 20,
        }}
      >
        ...
        {aggregateBuckets.length > 0 && (
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
              ...
              <span
                style={{ fontSize: 11, color: 'var(--color-muted-foreground)', marginLeft: 'auto' }}
              >
              ...
              {topProcesses.map(({ process, total }, rank) => {
                ...
                return (
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      <span
                        style={{ fontSize: 10, color: 'var(--color-muted-foreground)', width: 14 }}
                      >
                        #{rank + 1}
                      </span>
                      <span
      ...
      {/* ── Namespace groups ── */}
      {!loading &&
        byNamespace.map(([ns, nsProcesses]) => (
