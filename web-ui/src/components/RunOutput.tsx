...
export function RunOutput({ lines, exitCode, isRunning, error, onClear, height = 200 }: Props) {
    ...
    >
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontSize: 11, color: 'var(--color-muted-foreground)' }}>输出</span>
          {isRunning && (
            <span
              style={{
                display: 'inline-flex',
      ...
      >
        ...
        {error && (
          <div role="alert" style={{ color: 'var(--color-destructive)', marginBottom: 6 }}>
            {error}
          </div>
        )}
        {lines.map((line, i) => (
