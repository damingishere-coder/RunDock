...
export function projectActionError(response: ProjectActionResponse): string | null {
  ...
  const issues = failures.map(result => `${result.name}：${result.error ?? '操作失败'}`)
  if (response.persistence_error) {
    issues.push(`运行状态已改变，但保存失败：${response.persistence_error}`)
  }
  return issues.join('；') || '项目操作未完整完成'
}
