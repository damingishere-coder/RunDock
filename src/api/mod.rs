...
pub fn router(state: Arc<DaemonState>) -> Router {
    ...
    let stream_ticket = Router::new()
        .route("/stream-ticket", post(routes::auth::create_stream_ticket))
    ...
    let protected = Router::new()
        ...
        .nest(
            "/system",
            routes::system::router(Arc::clone(&state))
                .merge(routes::ui_settings::router(Arc::clone(&state))),
        )
        .nest("/ecosystem", routes::ecosystem::router(Arc::clone(&state)))
        .nest("/scripts", routes::scripts::router(Arc::clone(&state)))
        .nest(
            "/notifications",
            routes::notifications::router(Arc::clone(&state)),
        )
        .nest("/ai", routes::ai::router(Arc::clone(&state)))
        .nest("/telegram", routes::telegram::router(Arc::clone(&state)))
        .nest("/ports", routes::ports::router(Arc::clone(&state)))
        .nest("/tunnels", routes::tunnels::router(Arc::clone(&state)))
        .nest(
            "/terminals",
            routes::terminal::router(Arc::clone(&state))
                .merge(routes::terminal_history::router(Arc::clone(&state))),
        )
        .merge(routes::metrics::router(Arc::clone(&state)))
        .merge(routes::log_alerts::router(Arc::clone(&state)))
